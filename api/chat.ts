...
async function aiFunc1(query: string, historyText: string): Promise<string> {
  const ai = getGeminiClient();
  const prompt = `You're a helpful assistant of a food service company called FoodFix.
Please respond to the user's query courteously.
Use the following policy document -
${policyDocument}
If the question is related to policy then only answer it else say that I'm routing to a human support agent.
...
${historyText}`;
  ...
  return response.text || "I'm routing to a human support agent.";
}

async function aiFunc2(query: string, imageBase64: string, mimeType: string, historyText: string): Promise<string> {
  const ai = getGeminiClient();
  const prompt = `You're a helpful assistant of a food service company called FoodFix.
Please respond to the user's query courteously.
Use the following policy document -
${policyDocument}
Check the food quality and if the food quality is bad (food is burnt, mouldy, spoiled, or visibly unsafe), tell the customer that a refund is being processed and apologize.
Here is the query - ${query}.
Use the following historical conversation -
${historyText}`;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { query, image, mimeType, history } = body;
    ...
    if (image) {
      const reply = await aiFunc2(query, image, mimeType, historyText);
    ...
    const reply = await aiFunc1(query, historyText);
    return res.status(200).json({ text: reply });
  } catch (error: any) {
